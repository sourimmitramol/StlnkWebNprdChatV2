...
def extract_ocean_bl_number(text: str) -> Optional[str]:
    ...
    Handles:
        - “ocean bl 123456789”
        - “bill of lading 123456789”
        - raw pattern “123456789” (6+ digits)
    Returns the cleaned number or None.
    """
    patterns = [
        r'ocean bl\s*([0-9]{6,})',            # with the phrase “ocean bl”
        r'bill of lading\s*([0-9]{6,})',      # with the phrase “bill of lading”
        r'\b([0-9]{6,})\b'                    # just the raw number (6+ digits)
    ]
    for pat in patterns:
        m = re.search(pat, text, flags=re.IGNORECASE)
