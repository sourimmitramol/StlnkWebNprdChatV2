...
def ask(body: QueryWithConsigneeBody):
    ...
    def _sanitize_intermediate_steps(intermediate_steps):
        """Remove parser/_Exception artifacts before returning them to clients."""
        if not intermediate_steps:
            return []
        cleaned = []
        for step in intermediate_steps:
            # LangChain usually uses (AgentAction, observation)
            if isinstance(step, tuple) and len(step) == 2:
                action, observation = step
                tool_name = (getattr(action, "tool", None) or getattr(action, "tool_name", None) or "").strip()
                if tool_name == "_Exception":
                    continue
                # Drop common parsing error strings if they leak into observations
                if isinstance(observation, str) and "Invalid Format:" in observation:
                    continue
                cleaned.append(step)
                continue

            # Some serializers may convert steps to dicts
            if isinstance(step, dict):
                ...
                tool_name = (tool_name or step.get("tool") or "").strip()
                if tool_name == "_Exception":
                    continue
                obs = step.get("observation")
                if isinstance(obs, str) and "Invalid Format:" in obs:
                    continue
                cleaned.append(step)
                continue
    ...
    try:
        ...
        milestone_obs = _extract_milestone_observation(intermediate_steps)
        ...
        observation = _sanitize_intermediate_steps(intermediate_steps)
