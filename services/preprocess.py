...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    warnings.filterwarnings(
        "ignore", message=r"Could not infer format", category=UserWarning
    )
    warnings.filterwarnings(
        "ignore", message=r"In a future version of pandas", category=FutureWarning
    )
    ...
    if "container_number" in df.columns:
        df["container_number"] = (
            ...
            .astype(str)
            .apply(
                lambda x: re.sub(r"[^A-Z0-9]", "", x.upper()) if pd.notnull(x) else x
            )
        )
    ...
    if "eta_dp" in df.columns and (
        "ata_dp" in df.columns or "derived_ata_dp" in df.columns
    ):
        # Ensure columns exist, fill missing with NaT for safety in apply
        if "ata_dp" not in df.columns:
            df["ata_dp"] = pd.NaT
        if "derived_ata_dp" not in df.columns:
            df["derived_ata_dp"] = pd.NaT

        df["delay_days"] = df.apply(
            lambda row: delta_dp(row["eta_dp"], row["ata_dp"], row["derived_ata_dp"]),
            axis=1,
        )
