...
from agents.tools import handle_non_shipping_queries  # Add this
...
def validate_consignee_filtering(consignee_code: list) -> bool:
    ...
    try:
        ...
        if df_filtered.empty:
            logger.warning(
                f"No data found after consignee filtering for codes: {consignee_code}"
            )
            return False
...
def route_query(
    query: str, consignee_code: list = None, chat_history: list = None
) -> str:
    ...
    try:
        ...
        # Default fallback - try non-shipping handler for greetings/company info
        return handle_non_shipping_queries(query, chat_history=chat_history)
