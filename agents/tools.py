...
def get_hot_upcoming_arrivals(query: str = None, consignee_code: str = None, **kwargs):
    ...
    Uses the Analytics Engine for precise date and filter handling.
    """
    from .analytics_engine import unified_shipment_analyst

    q = (query or kwargs.get("question") or kwargs.get("input") or "").strip()

    # Ensure the query explicitly mentions "hot/priority"
    if not any(
        word in q.lower() for word in ["hot", "priority", "urgent", "expedited", "rush"]
    ):
        q = f"{q} (filter for hot/priority shipments)"

    return unified_shipment_analyst(q)
...
def get_hot_containers(
    ...
    """
    Unified hot-shipment handler using the Analytics Engine.
    Handles 'hot', 'priority', 'urgent', or 'expedited' containers, POs, and BLs.
    Supports filters for:
    - Time windows (next 3 days, this week, etc.)
    - Transport modes (sea, air, road, etc.)
    - Locations (ports, cities, codes)
    - Delay conditions (delayed by N days)
    """
    from .analytics_engine import unified_shipment_analyst

    query = question or kwargs.get("question") or kwargs.get("input") or ""

    # Ensure the query explicitly mentions "hot/priority" even if it was implicit in tool routing
    if not any(
        word in query.lower()
        for word in ["hot", "priority", "urgent", "expedited", "rush"]
    ):
        query = f"Retrieve records for: {query} (filter for hot/priority shipments)"

    # Delegate to the analytics engine
