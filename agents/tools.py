...
def get_containers_PO_OBL_by_supplier(query: str) -> str:
    ...
    **CRITICAL FIX**: This function now correctly handles supplier lookup queries
    ...
    try:
        logger.info(f"[get_containers_PO_OBL_by_supplier] Extracted: container={container_no}, po={po_no}, obl={obl_no}, is_lookup={is_lookup_query}")
    except:
        pass
    ...
    if is_lookup_query or container_no or po_no or obl_no:
            ...
            if po_no:
                ...
                if "supplier_vendor_name" in rows.columns:
                    ...
                    supplier_groups = rows.groupby("supplier_vendor_name").agg({
                        "container_number": lambda x: ", ".join(sorted(set(x.dropna().astype(str)))),  # **FIXED**: Join as string
                    ...
                    for _, group in supplier_groups.iterrows():
                        results.append({
                            ...
                            "container_numbers": group["container_number"],  # **FIXED**: Now a comma-separated string
                            "discharge_port": group["discharge_port"],
                            "consignee_code_multiple": group["consignee_code_multiple"]
                        })
                    
                    try:
                        logger.info(f"[get_containers_PO_OBL_by_supplier] Returning {len(results)} supplier record(s) for PO {po_no}")
                    except:
                        pass
        ...
        if container_no:
            ...
            if "supplier_vendor_name" in rows.columns:
                supplier_groups = rows.groupby("supplier_vendor_name").agg({
                    "po_number_multiple": lambda x: ", ".join(sorted(set(x.dropna().astype(str)))),
                    "discharge_port": "first",
                    "consignee_code_multiple": "first"
                }).reset_index()
        ...
        if obl_no:
            ...
            if "supplier_vendor_name" in rows.columns:
                supplier_groups = rows.groupby("supplier_vendor_name").agg({
                    "container_number": lambda x: ", ".join(sorted(set(x.dropna().astype(str)))),
    ...
    has_status_filter = any(w in q_lower for w in ["delay", "late", "upcoming", "arriving", "in transit", "arrived"])
...
def get_hot_containers(question: str = None, consignee_code: str = None, **kwargs) -> str:
    ...
    # -----------------------
    # Optional final_carrier_name (minimal + query-driven)
    # -----------------------
...
def get_upcoming_pos(query: str, consignee_code: str = None) -> str:
    ...
    if 'discharge_port' not in candidate_df.columns:
        try:
            logger.warning("[get_upcoming_pos] discharge_port column not found in dataset")
        except:
            pass
    else:
        ...
        if paren_match:
            tok = paren_match.group(1)
            location_mask = candidate_df['discharge_port'].astype(str).str.upper().str.contains(
                rf'\({re.escape(tok)}\)', 
                na=False
            )
            ...
            if location_mask.any():
                ...
                location_name = tok
                try:
                    logger.info(f"[get_upcoming_pos] Found port code in parentheses: {tok}")
                except:
                    pass
        ...
        if not location_found:
            ...
            if bare_code_match:
                ...
                if location_mask.any():
                    ...
                    location_name = tok
                    try:
                        logger.info(f"[get_upcoming_pos] Found BARE port code: {tok} (matched in discharge_port)")
                    except:
                        pass
                ...
                if city_match:
                    ...
                    if city and len(city) > 2:
                        ...
                        city_mask = candidate_df['discharge_port'].apply(
                            lambda s: city_norm in normalize_port_name(s) if pd.notna(s) else False
                        )
                        ...
                        if city_mask.any():
                            ...
                            location_found = True
                            try:
                                logger.info(f"[get_upcoming_pos] Found city: {city} in discharge_port column ONLY")
                            except:
                                pass
                            break
        ...
        if location_found:
            ...
            try:
                logger.info(f"[get_upcoming_pos] After discharge_port filter ({location_name}): {len(candidate_df)} rows")
                # Log sample discharge ports to verify
                sample_ports = candidate_df['discharge_port'].head(5).tolist()
                logger.info(f"[get_upcoming_pos] Sample discharge ports after filter: {sample_ports}")
            except:
                pass
...
def get_containers_with_etd_delay(question: str = None, consignee_code: str = None, **kwargs) -> str:
    """
    ...
    if consignee_code and "consignee_code_multiple" in df.columns:
        ...
        try:
            logger.info(f"[get_containers_with_etd_delay] After consignee filter: {len(df)} rows")
        except:
            pass
    ...
    try:
        logger.info(f"[get_containers_with_etd_delay] After filtering to departed containers: {len(df_departed)} rows")
    except:
        pass
    ...
    try:
        ...
        sample_delays = df_departed[["container_number", "etd_lp", "atd_lp", "etd_delay_days"]].head(10)
    ...
    try:
        logger.info(f"[get_containers_with_etd_delay] Parsed delay filter: threshold={delay_threshold}, operator={delay_operator}")
    except:
        pass
    ...
    if delay_threshold is not None:
        if delay_operator == 'range':
            ...
            delay_mask = (df_departed["etd_delay_days"] >= low) & (df_departed["etd_delay_days"] <= high)
            filter_desc = f"ETD delay between {low} and {high} days"
        elif delay_operator == '<':
            delay_mask = (df_departed["etd_delay_days"] > 0) & (df_departed["etd_delay_days"] < delay_threshold)
    ...
    try:
        logger.info(f"[get_containers_with_etd_delay] After delay filter ({filter_desc}): {len(results)} rows")
    except:
        pass
    ...
    load_port = None
    load_port_cols = [c for c in ["load_port", "final_load_port"] if c in results.columns]
    ...
    if load_port_cols:
        ...
        for pattern in port_patterns:
            ...
            if port_match:
                ...
                load_port = re.sub(r'\s+(WITH|BY|FOR|IN|ETD|DELAY|DELAYED)$', '', load_port, flags=re.IGNORECASE).strip()
                break
        
        if load_port:
            try:
                logger.info(f"[get_containers_with_etd_delay] Extracted load_port: '{load_port}'")
            except:
                pass
            ...
            load_port_norm = normalize_port_name(load_port)
            results['_norm_port'] = results[load_port_cols[0]].apply(normalize_port_name)
            ...
            if user_code:
                ...
                try:
                    logger.info(f"[get_containers_with_etd_delay] Code match '{user_code}': {port_mask.sum()} rows")
                except:
                    pass
            ...
            if not port_mask.any():
                ...
                if user_words:
                    ...
                    try:
                        logger.info(f"[get_containers_with_etd_delay] Strict word match for '{load_port_norm}': {port_mask.sum()} rows")
                    except:
                        pass
            ...
            if port_mask.any():
                ...
                try:
                    logger.info(f"[get_containers_with_etd_delay] After port filter: {len(results)} rows")
                    if len(results) > 0:
                        sample_ports = results[load_port_cols[0]].head(5).tolist()
                        logger.info(f"[get_containers_with_etd_delay] Sample matched ports: {sample_ports}")
                except:
                    pass
            else:
                ...
                try:
                    logger.warning(f"[get_containers_with_etd_delay] No containers found from load port '{load_port}'")
                except:
                    pass
            
            # Clean up temporary columns
            results.drop(columns=['_norm_port', '_port_code'], inplace=True, errors='ignore')

    # Transport mode filter
    modes = extract_transport_modes(query)
    if modes and 'transport_mode' in results.columns:
        mode_mask = results['transport_mode'].astype(str).str.lower().apply(lambda s: any(m in s for m in modes))
        results = results[mode_mask].copy()
        filters_applied.append(f"by {', '.join(modes)}")
        
        try:
            logger.info(f"[get_containers_with_etd_delay] After transport mode filter: {len(results)} rows")
        except:
            pass
    ...
    if is_hot_query:
        ...
        if hot_flag_cols:
            ...
            try:
                logger.info(f"[get_containers_with_etd_delay] After hot filter: {len(results)} rows")
            except:
                pass

    # Date range filter (on ATD_LP)
    if re.search(r'\b(in|during|last|this)\s+(month|week|january|february|march|april|may|june|july|august|september|october|november|december)', query, re.IGNORECASE):
        try:
            ...
            try:
                logger.info(f"[get_containers_with_etd_delay] After date range filter: {len(results)} rows")
            except:
                pass
        except Exception:
            pass
...
def get_containers_by_final_destination(query: str) -> str:
    ...
    **CRITICAL**: Uses ONLY eta_fd column for final destination ETA calculations.
    ...
    try:
        logger.info(f"[get_containers_by_final_destination] Time window: {start_date} to {end_date} ({period_desc})")
    except:
        pass
    ...
    try:
        logger.info(f"[get_containers_by_final_destination] Initial dataset: {len(df)} rows")
    except:
        pass
    ...
    if fd_location:
        ...
        time_words = ['in', 'next', 'this', 'within', 'by', 'last', 'week', 'month', 'days', 'day']
        for word in time_words:
            fd_location = re.sub(rf'\b{word}\b', '', fd_location, flags=re.IGNORECASE).strip()
        fd_location = re.sub(r'\s+', ' ', fd_location).strip()
    ...
    try:
        logger.info(f"[get_containers_by_final_destination] Extracted FD location: '{fd_location}', code: '{fd_code}'")
    except:
        pass
    ...
    if fd_code:
        ...
        try:
            logger.info(f"[get_containers_by_final_destination] Code match strategy: {code_mask.sum()} matches")
        except:
            pass
    ...
    if not fd_mask.any() and fd_location:
        ...
        try:
            logger.info(f"[get_containers_by_final_destination] Exact name match: {exact_mask.sum()} matches")
        except:
            pass
    ...
    if not fd_mask.any() and fd_location:
        ...
        if fd_words:
            ...
            try:
                logger.info(f"[get_containers_by_final_destination] Word-based match: {word_mask.sum()} matches")
            except:
                pass
    ...
    if not fd_mask.any() and fd_location:
        ...
        if close_matches:
            ...
            try:
                logger.info(f"[get_containers_by_final_destination] Fuzzy match: {fuzzy_mask.sum()} matches, top matches: {close_matches[:3]}")
            except:
                pass
    ...
    try:
        logger.info(f"[get_containers_by_final_destination] Found {len(df)} containers after FD filter")
    except:
        pass
    ...
    try:
        logger.info(f"[get_containers_by_final_destination] Found {len(results)} containers after date filter (using eta_fd only)")
    except:
        pass
    ...
    try:
        logger.info(f"[get_containers_by_final_destination] Returning {len(out)} containers")
    except:
        pass
...
def get_cargo_ready_date(query: str) -> str:
    ...
    if 'cargo_ready_date' in result_df.columns:
        ...
        result_df['cargo_ready_date'] = result_df['cargo_ready_date'].apply(parse_cargo_date)
        logger.debug(f"[get_cargo_ready_date] Parsed cargo_ready_date: {result_df['cargo_ready_date'].notna().sum()} non-null values")
    ...
    if po_numbers:
        ...
        if 'po_number_multiple' in result_df.columns:
            ...
            result_df = result_df[result_df['po_number_multiple'].apply(lambda x: po_matches(x, po_list))]
    ...
    if container:
        ...
        if 'container_number' in result_df.columns:
            result_df = result_df[
                result_df['container_number'].astype(str).str.upper().str.strip() == container_norm.upper()
            ]
    ...
    if any(kw in query.lower() for kw in supplier_keywords):
        ...
        supplier_pattern = re.search(r'(?:supplier|shipper|vendor|from)\s+([A-Za-z0-9\s&\-\.]+?)(?:\s+|$|in\s|at\s|for\s)', query, re.IGNORECASE)
        if supplier_pattern and 'supplier_vendor_name' in result_df.columns:
            supplier_name = supplier_pattern.group(1).strip()
            result_df = result_df[
                result_df['supplier_vendor_name'].astype(str).str.contains(supplier_name, case=False, na=False)
            ]
    ...
    # ------------------------------------------------------------------
    port_pattern = re.search(r'(?:port|to|at|discharge|destination)\s+([A-Z]{2,6}|[A-Za-z\s]+?)(?:\s|$)', query, re.IGNORECASE)
    if port_pattern and 'discharge_port' in result_df.columns:
        port_name = port_pattern.group(1).strip()
        result_df = result_df[
            result_df['discharge_port'].astype(str).str.contains(port_name, case=False, na=False)
        ]
    ...
    try:
        ...
        if any(keyword in query.lower() for keyword in ['next', 'last', 'within', 'in', 'during', 'between']):
    ...
    # ------------------------------------------------------------------
    load_port_pattern = re.search(r'(?:from\s+load\s+port|load\s+port|origin)\s+([A-Z]{2,6}|[A-Za-z\s]+?)(?:\s|$)', query, re.IGNORECASE)
    if load_port_pattern and 'load_port' in result_df.columns:
        load_port_name = load_port_pattern.group(1).strip()
        result_df = result_df[
            result_df['load_port'].astype(str).str.contains(load_port_name, case=False, na=False)
        ]
    ...
    # ------------------------------------------------------------------
    if result_df.empty:
        filter_desc = " with filters: " + ", ".join(filters_applied) if filters_applied else ""
    ...
    # ------------------------------------------------------------------
    logger.info(f"[get_cargo_ready_date] Returning {len(result_df)} records{' with filters: ' + ', '.join(filters_applied) if filters_applied else ''}")
