...
def get_hot_containers(
    ...
    **CRITICAL FIX**: Ensures ONLY containers with hot_container_flag = 'Y' or 'YES' are returned.
    ...
    if has_time_period:
        try:
            ...
            if eta_col:
                ...
                mask = (hot_df[eta_col] >= start_date) & (hot_df[eta_col] <= end_date)
    ...
    out_cols = [
        ...
        "po_number_multiple",
        "ocean_bl_no_multiple",
        "booking_number_multiple",  # Include hot flag for verification
        "discharge_port",
        "load_port",
        "revised_eta",
        "eta_dp",
    ...
    # Format dates
    date_cols = [c for c in ["revised_eta", "eta_dp", "ata_dp"] if c in result.columns]
    if date_cols:
...
def analyze_data_with_pandas(query: str) -> str:
    ...
    if "average delay" in query.lower():
        if "delay_days" not in df.columns:
            df["delay_days"] = (
                pd.to_datetime(df["ata_dp"], errors="coerce")
                - pd.to_datetime(df["eta_dp"], errors="coerce")
            ).dt.days
