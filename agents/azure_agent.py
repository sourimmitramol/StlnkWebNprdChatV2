...
def initialize_azure_agent(
    ...
    system_instructions = (
        ...
        "1. For ANY query about 'status', 'current status', 'milestones', 'tracking history', or 'movement history' of a SPECIFIC ID -> ALWAYS use 'Get Container Milestones'.\n"
        "2. For specific fields (who is the supplier, vendor name, latest ETA, carrier name, container type) or any analytical questions (counts, sums) -> ALWAYS use 'Analyze Data with Pandas'.\n"
        "3. NEVER use 'Analyze Data with Pandas' for 'status' or 'milestones'. These MUST go to 'Get Container Milestones'.\n"
        "4. For greetings, small talk, or general company info -> use 'Handle Non-shipping queries'.\n"
        "5. If 'Analyze Data with Pandas' returns no data but the query is about company info, failover to 'Handle Non-shipping queries'.\n"
        "6. Use 'Get Today Date' immediately for relative timing (today, next month, etc.) before choosing other tools.\n\n"
