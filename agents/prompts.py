...
def parse_time_period(query: str) -> Tuple[pd.Timestamp, pd.Timestamp, str]:
    ...
    if re.search(r"\b(this|current)\s+week\b", query, re.IGNORECASE):
        return (
            this_week_start,
            this_week_end,
            f"this week ({this_week_start.strftime('%Y-%m-%d')} to {this_week_end.strftime('%Y-%m-%d')})",
        )
    if re.search(r"\bnext\s+week\b", query, re.IGNORECASE):
        return (
            next_week_start,
            next_week_end,
            f"next week ({next_week_start.strftime('%Y-%m-%d')} to {next_week_end.strftime('%Y-%m-%d')})",
        )
    ...
    if re.search(r"\bthis\s+month\b", query, re.IGNORECASE):
        ...
        return (
            ...
            f"this month ({start.strftime('%Y-%m-%d')} to {end.strftime('%Y-%m-%d')})",
        )
    if re.search(r"\bnext\s+month\b", query, re.IGNORECASE):
        ...
        return (
            nm_start,
            nm_end,
            f"next month ({nm_start.strftime('%Y-%m-%d')} to {nm_end.strftime('%Y-%m-%d')})",
        )
    if re.search(r"\blast\s+month\b", query, re.IGNORECASE):
        ...
        return (
            pm_start,
            pm_end,
            f"last month ({pm_start.strftime('%Y-%m-%d')} to {pm_end.strftime('%Y-%m-%d')})",
        )
        ...
        if month:
            try:
                ...
                return (
                    dt.normalize(),
                    dt.normalize(),
                    f"{month_name.capitalize()} {day}, {year}",
                )
            except ValueError:
                pass
